...
| `MAX_STUCK_SKIPS` | 3 | 同じ操作の見送り（実行全体の合計）が、この回数を超えたら失敗にする。見送りは、同じステップの中で選び直すので、ステップを余計には使わない | — |
...
「同じ呼び出しが 3 回目」の 3 は、定数ではなく、`run_agent` の中の直値です（`attempts` が 2 以上）。3 回目は呼ばずに見送り、`history` に `SKIPPED` を足して、同じステップの中で選び直します（ページの読み直しはしません）。呼び出しに要素（ref。`browser_fill_form` の `fields[].target` も含む）があれば、その要素を、実行の最後まで（ページの URL・タイトルごとに）候補から外して、同じツールで引数を決め直します。要素がない呼び出し（`browser_navigate_back` など）や、ページの URL・タイトルが読めないページでは、そのツールを、そのステップの選択肢から除いて、ツールを選び直します。
