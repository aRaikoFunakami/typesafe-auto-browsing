...
| 長い記事から探す（Wikipedia） | 記事全体を、パートに分けて読む（記事が長いほど増える） | `browser_find` などで、必要な所だけを取れば、少なく済む（typesafe では、下の「browser_find で費用を下げる試み」の制限がある） | **2〜4 倍** |
...
## browser_find で費用を下げる試み（調べたが、ここでは実施しない）

Claude Code が Wikipedia の記事で安く済んだのは、`browser_find` で必要な所だけを取ったからでした（上の ③）。typesafe-auto-browsing でも同じことができるかを、別の worktree（ブランチ `find-focus`、`main` には入れていません）で実験しました。結論は、**効く範囲が狭く、制限が多いので、ここでは取り入れません。**

### 試したこと

1. **先頭だけ見せる（`--lazy-page`）:** 長いページ（5 万文字超）で、全パートを判定せず、先頭の約 8,000 文字だけを TypeSafe に見せ、末尾に `... (rest of the page not shown: N chars) ...` を付ける。
2. **質問文に 1 文足す:** ページが一部だけのとき、`tool` の質問に「`page` は一部かもしれない。`browser_find` はページ全体を探して、一致した所だけを返す」を足す。
3. **`--done-threshold 0.9`:** 先頭だけを見た段階で終わらず、`browser_find` の結果を見てから終わらせる。

### 結果

**東京タワーの記事（設計者を読む）:**

| 設定 | `browser_find` を選ぶ確率 | `find` が実行された | コスト（検索から / URL 直接） |
|---|---|---|---|
| 現状（全パート判定） | – | – | $0.0154 / $0.0109 |
| 先頭だけ | 0.64〜0.70（URL 直接） | 2/3、0/3 | $0.0009〜$0.0021 / $0.0003 |
| + 質問文の 1 文 | 0.97（URL 直接）、0.82〜0.87（検索から） | 1/3、0/3 | $0.0009〜$0.0016 / $0.0003 |
| + しきい値 0.9 | 0.97、0.87〜0.89 | **3/3、3/3** | **$0.0016〜$0.0017 / $0.00085** |

- 各 3 回。最後の設定で、6 回すべてで `find` が実行され、答え（内藤多仲）が TypeSafe に見えて終わりました。現状の約 1/9〜1/13、Claude Code（$0.0273）の約 1/16〜1/32 です。

**Amazon（最安の USB-C ケーブル）:**

| 設定 | 結果 | コスト |
|---|---|---|
| 現状（全パート判定） | **成功**（4 ステップ、107 リクエスト） | $0.0309 |
| 先頭だけ + 文 + しきい値 0.9（2 回） | **失敗**（`stuck: repeated browser_snapshot`） | $0.0114 |
| 先頭だけ + 文（1 回） | **失敗**（同上） | $0.0114 |
| 先頭だけ（文なし、2 回） | **失敗**（ステップ上限） | $0.019 |

### typesafe で browser_find を使う場合の制限

1. **`text` は、目的文の語か、見えているページの文字列から選ぶしかない。** TypeSafe は文字列を作れません。目的文に探す語があれば（「設計者」「高さ」「完成」）、ページを見なくても選べます。ない場合（Amazon の「並べ替え」「価格: 安い順」）は、その語がページに見えている必要があり、結局、ページの関連する部分を読むことになります。先頭だけでは見えず、`browser_find cannot be used now: nothing to specify` になりました。Claude Code は、その語を自分の知識から作れます。
2. **ツールの説明が、この使い方を想定していない。** MCP の `browser_find` の説明は「要素とその ref を探すだけなら、スナップショット全体を取るより安い」で、質問文も「the current `page`」と、ページが見えていることを前提にしています。何も足さないと、ページを外したとき、`browser_snapshot`（ページを取る）が最有力になります。質問文に 1 文足すと、`find` を選ぶ確率は上がりますが、これは MCP の説明をそのまま使う方針からの逸脱です。
3. **`done` が先に効く。** 同じステップで `done` と `tool` を聞き、`done` がしきい値以上なら、選んだツールを呼ばずに終わります。先頭だけを見た段階で、`done` が 0.80〜0.81（しきい値 0.8）になり、`find` を実行しないで終わる例が多くありました（文のみの 6 回中 5 回）。`--done-threshold` を上げると防げますが、実行全体に効き、他の目的への影響は調べていません。
4. **先頭だけ見せるのは、コードによる切り捨て。** 「どこを見るか」を TypeSafe が判断せず、コードが機械的に切る処理です。操作する部品がページの奥にある場合（Amazon の並べ替え）は、見つけられず、失敗します。現状の全パート判定は、そのための仕組みです。
5. **`browser_find` の出力は、見せる量に上限がある。** TypeSafe に見せるのは先頭の 8,000 文字（`FOCUS_CHARS`）までです。一致が多い語（「高さ」は 36 件、約 2.4 万文字）では、答えの行が切れる可能性があります。
6. **効く目的が限られる。** 効くのは、「目的文に探す語が入っている、事実を読む目的」（長い記事から 1 つの事実を探す）だけです。`prompts/` の 11 件では Wikipedia の 3 件で、合計は今 約 $0.038（11 件の合計 $0.115 の約 1/3）です。PyPI（ページが短い）、HN（全体を比べる）、Yahoo と Amazon（操作する）には効きません。
7. **Claude Code との構造の違い。** Claude Code の Playwright MCP は、`browser_navigate` などの結果に、ページの内容を入れず、スナップショットのファイルへのリンクだけを返します。だから、読まないと決めれば安く済みます。typesafe-auto-browsing は、毎ステップ全ページのスナップショットを取り、その内容を TypeSafe に見せる作りです。

### 実施しない理由

- 効く範囲が狭い（上の 6）。
- 効かせるには、「コードが見せる範囲を切る」「質問文を足す」「しきい値を変える」という、TypeSafe に判断させる方針から外れる変更が要る（上の 2、3、4）。
- 判定を誤ると、Amazon のように失敗する（上の 1、4）。

実験のコードは、ブランチ `find-focus`（worktree `.claude/worktrees/find-focus`、未コミット）にあります。`main` と `answer-candidates-and-docs` には入っていません。将来、やるとしたら、「事実を読む目的で、探す語が目的文にあるか」を Noul で判定し、詰まったら全パート判定に戻す形が考えられますが、実装も検証もしていません。
